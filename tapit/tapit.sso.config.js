const TAPIT_SSO_CONFIG = {
  /* String type parameter to indicate the application that is sending the request. */
  origin: 'web',
  project: 'web' /*it will be deprecated*/,
  /* String type parameter for the logo image URL or the base64 image. */
  logoUrl: 'data:image/png;base64,',
  /* String type parameter for the slogan in HTML tag format. Example: <p>Our Slogan</p>. */
  sloganHtml: '<p class="sso-text-center sso-text-xs">La comunidad de los <span class="sso-text-primary-500">amantes de la cerveza.</span></p>',
  /* Boolean type parameter to show the signIn facebook button. */
  showFacebookButton: true ,
  showLoginFacebookButton: true /*it will be deprecated*/,
  showSignUpFacebookButton: true /*it will be deprecated*/,
  /* Boolean type parameter to show the sign form. */
  showSignUpForm: true,
  showSignUPWithFields: true /*it will be deprecated*/,
  /* Boolean type parameter to show offers options. */
  showOffersOption: true,
  /* String type parameter for sample mail to be displayed as a value within the email field. */
  loginEmail: '',
  /* String type parameter to show a suggestion of mail or text under the email field. Empty by default */
  loginEmailHint: '',
  /* String type parameter to show a suggestion of password or text under the password field. Empty by default */
  loginPasswordHint: '',
  /* String type parameter for legend of acceptance of terms and conditions when the user registers, in HTML tag format. SSO can recognize and apply tailwind css classes */
  signUpAcceptTermsHtml: '<h1 class="sso-text-sm sso-font-bold sso-mb-1 sso-text-left">Al registrarte estás aceptando:</h1><p class="sso-text-sm sso-text-left">Nuestros <a class="sso-text-primary-500" href="/terms.html" target="_blank">Términos y Condiciones de Tapit,</a> las <a class="sso-text-primary-500" href="/privacy.html" target="_blank">Políticas de Privacidad,</a> la <a class="sso-text-primary-500" href="/data-proteccion.html" target="_blank">Política de protección de datos personales.</a> <br><br> El procesamiento de mi información personal por parte de Bavaria & Cía S.C.A. con las finalidades y usos requeridos por cerveza Poker descritos en la mencionada política. <br><br> Confirmo ser mayor de 18 años.</p>',
  /* String type parameter for legend of acceptance of sending offers when the user registers, in HTML tag format. SSO can recognize and apply tailwind css classes */
  signUpAcceptOffersHtml: '<p class="sso-text-sm sso-text-left">Acepta para recibir actualizaciones sobre el mundo de la cerveza y obtén nuestras ofertas exclusivas! Sólo las enviamos a nuestros #BeerLovers (Te prometemos no enviar spam)</p>',
  /* String type parameter for legend of acceptance of terms and conditions when the user registers, in HTML tag format. SSO can recognize and apply tailwind css classes */
  loginFooterHtml: '<p class="sso-text-sm sso-text-center">Al iniciar sesión estás aceptando nuestros <a class="sso-text-primary-500" href="/terms.html" target="_blank">Términos y Condiciones de Tapit,</a> las <a class="sso-text-primary-500" href="/privacy.html" target="_blank">Políticas de Privacidad,</a> la <a class="sso-text-primary-500" href="/data-proteccion.html" target="_blank">Política de protección de datos personales</a> y el envío de ofertas exclusivas a mi correo. </p>',
  /* Boolean type parameter to show the close button. */
  showCloseButton: true,
  /* Boolean type parameter to show the container background. */
  showContainerBackground: true,
  /* String type parameter to define the language, in case a valid parameter is not sent, the browser language will be taken */
  language: 'es',
  /* String type parameter to define the referrer of the iframe (the domain of the current client)*/
  reference: '',
  /* Boolean type parameter to show recover Password link. */
  showRecoverPassword: true,
  /*  String type parameter to add custom stylest in CSS format. Empty by default */
  styles: '.text-primary-500{color: #da291c;}',
  /* Object type parameter to add custom inputs.
      Each custom input must be made up of a key and a label.
      The password will be saved in the database in the interests property, only if the user selects that interest
  */
  interests: [{key: 'beer', label: 'Cerveza'}, {key: 'soccer', label: 'Futbol'}],
  interestsMandatory: true,
  showGoogleButton: true,
  showIdentityTypeInput: true,
  defaultIdentityType: 'COL_CC',
  identityTypes: [{label: 'CC', key: 'COL_CC'}, {label: 'CPF', key: 'BR_CPF'}],
  showIdentityNumberInput: true,
  showPhoneNumberInput: true,
  showGenderInput: true,
  showCityInput: true,
  cities: [
    {label: 'BOGOTÁ', key: 'bogota'},
    {label: 'MEDELLIN', key: 'medellin'},
    {label: 'CARTAGENA', key: 'cartagena'}
  ],
  disableFacebookSignup: false
}
